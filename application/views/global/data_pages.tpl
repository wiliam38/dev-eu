...
{if $paginate|default:'' != '' && $paginate.total_pages > 0}
	{if $paginate.show_pages < 3}{$paginate.show_pages=3}{/if}

	{if $paginate.page < $paginate.show_pages|default:5}
		{$start=1}
		...
		{/if}
	{elseif $paginate.page > $paginate.total_pages-$paginate.show_pages|default:5 + 1}
		{if $paginate.show_pages|default:5 + 1 >= $paginate.total_pages}
			{$start=1}
		{else}
		...
		{$loop=$paginate.total_pages + 1}	
	{else}
		{if $paginate.page - floor($paginate.show_pages|default:5 / 2) - 1 == 1}
			{$start=1}
		{else}
			{$start=$paginate.page - floor($paginate.show_pages|default:5 / 2)}
		{/if}
		{if $paginate.page + floor($paginate.show_pages|default:5 / 2) + 1 == $paginate.total_pages}
			{$loop=$paginate.total_pages + 1}
		{else}
	...
	<div class="data_pages" id="{$data_page_name}">
		...
		{if $start != '1'}
			<a href="{$href|default:'#none'}?{$param}=1" class="item" data-page="1" onclick="return false;">1</a>
			...
		{/if}			
		...
		{if $loop - 1 != $paginate.total_pages}
			...
			<a href="{$href|default:'#none'}?{$param}={$paginate.total_pages}" class="item" data-page="{$paginate.total_pages}" onclick="return false;">{$paginate.total_pages}</a>
		{/if}
