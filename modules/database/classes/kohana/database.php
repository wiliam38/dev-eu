	...
	public function quote_column($column)
	{
		...
		else
		{
			...
			elseif (strpos($column, '"') !== FALSE)
			{
				...
				//$column = preg_replace_callback('/"(.+?)"/', create_function ('$matches', 'return $this->quote_column($matches[2]);'), $column);
				while (preg_match('/"(.+?)"/', $column, $matches)) {
					$column = str_replace('"'.$matches[1].'"', $this->quote_column($matches[1]), $column);
				}
			}
			elseif (strpos($column, '.') !== FALSE)
			{
