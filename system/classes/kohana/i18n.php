...
class Kohana_I18n {
	...
	public static function get($string, $lang = NULL)
	{
		...
		// ISS update
		if (!isset($table[$string]) && substr($lang, 0, 4) == 'site' && preg_match('/.*\..*/', $string)) {
			$resources = Model::factory('manager_resources');		
			$resources->clear_cache();	
			$table = I18n::load($lang);
			
			if (!isset($table[$string])) {
				if (Kohana::$environment == Kohana::DEVELOPMENT) trigger_error("I18n: missing translation '".$string."' value");
				else $val = '';	
			}
		}			
