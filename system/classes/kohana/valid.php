...
class Kohana_Valid {
	...
	{
		// ISS
		if (strtolower(substr($url,0,7)) != 'http://' && strtolower(substr($url,0,8)) != 'https://') $url = 'http://'.$url;
	...
	{
		return (preg_match('/^(\+)?[\s|\d|\.|-]*$/', $number) && strlen(preg_replace('/\D+/', '', $number)) >= $lengths);
	}
	...
	public static function date($str)
	{
		if (is_array($str)) {
			...
			for($i=0; $i<count($str); $i++) {
				if (strtotime($str[$i]) === FALSE) $result = false;
			}
			return $result;
		} else {
	...
	public static function numeric($str)
	{
		...
		if (is_array($str)) {
			$result = true;
			for ($i=0; $i<count($str); $i++) {
				if (!(bool)preg_match('/^-?+(?=.*[0-9])[0-9]*+'.preg_quote($decimal).'?+[0-9]*+$/D', (string) $str[$i])) $result = false;
			}
			return $result;
		} else {
