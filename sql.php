...
$conf = require_once 'application/config/database.php';

if (!isset($conf['default']['db_admin']) || $conf['default']['db_admin'] != true) {
	header('Location: .');
	exit();
}

 $ACCESS_PWD=$conf['default']['connection']['password']; #!!!IMPORTANT!!! this is script access password, SET IT if you want to protect you DB from public access
 ...
 'host'=>$conf['default']['connection']['hostname'],#optional
 'port'=>"",#optional
 'chset'=>$conf['default']['charset'],#optional, default charset
 );
...
<br>";
 ...
 if ($is_sht) $headers.="<th>engine</th><th>~rows</th><th>data size</th><th>index size</th><th>show create table</th><th>explain</th><th>indexes</th><th>export</th><th>optimize</th><th>repair</th>";
 $headers.="</tr>\n";
 $sqldr.=$headers;
 $swapper=false;
...
<div class="inv">
<span style="padding-left: 5px; font-weight: bold;">SQL Admin</span>
...
<div class="frm">
<label class="l">Username:</label><input type="text" name="v[user]" value="<?php echo $DB['user']?>" style="width: 290px;"><br>
<label class="l">Password:</label><input type="password" name="v[pwd]" value="" style="width: 290px;"><br>
<label class="l">Database:</label><input type="text" name="v[db]" value="<?php echo $DB['db']?>" style="width: 290px;"><br>
...
function loadcfg(){
 ...
 $_SESSION['DB']['host']=$DBDEF['host'];
 $_SESSION['DB']['port']=$DBDEF['port'];
}
